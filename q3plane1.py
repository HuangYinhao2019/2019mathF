...
def getPoint():
    ...
    return Point
def issuccess(T,p,a1=25, a2=15, b1=20, b2=25, o=30, l=0.001):
    v = 0
    h = 0
    count_p = 0
    ...
    for i in range(int(math.pow(2,count_m)+0.2)):
        ...
        for j in range(count_m):
            if bi % 2 == 1:
                p[j] = 1
            bi = int(bi / 2)
        print(p,issuccess(T,p),round(math.pow(0.2,np.sum(p)) * math.pow(0.8,count_m-np.sum(p)),5))
...
ans_list, h_list, v_list = pro_improve(25, 15, 20, 25, 30, 0.001)
print(ans_list)
